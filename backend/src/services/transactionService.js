...
class TransactionService {
  ...
  static getById(transactionId, userId) {
    try {
      ...
      if (!transaction || transaction.userId !== userId) {
        return { success: false, message: ERROR_MESSAGES.TRANSACTION_NOT_FOUND };
      }
  ...
  static update(transactionId, userId, updates) {
    try {
      ...
      NotificationService.notify(EVENT_TYPES.TRANSACTION_UPDATED, { transaction: updatedTransaction, userId });
      logger.info(`Transaction mise à jour: ${transactionId}`);

      return { success: true, transaction: updatedTransaction, message: SUCCESS_MESSAGES.TRANSACTION_UPDATED };
    } catch (error) {
  ...
  static delete(transactionId, userId) {
    try {
      ...
      NotificationService.notify(EVENT_TYPES.TRANSACTION_DELETED, { transactionId, userId });
...
TransactionService._instance = null;
