...
class BudgetService {
  constructor() {
    if (BudgetService._instance) {
      return BudgetService._instance;
    }
    BudgetService._instance = this;
  }
    ...
    try {
      ...
      return {
        ...
        summary: {
          ...
          isOverBudget: user.monthlyBudgetLimit > 0 && totalExpense > user.monthlyBudgetLimit,
          period: {
  ...
  static getCategoryBreakdown(userId) {
    try {
      ...
      return { success: true, breakdown, total: totalExpense };
    } catch (error) {
    ...
    try {
      ...
      if (balance > 0) {
        recommendations.push({
          type: 'positive',
          message: 'Excellent travail ! Votre solde est positif. Continuez à gérer vos dépenses.',
          icon: '✓',
        });
        if (balance > 5000) {
          recommendations.push({
            type: 'savings',
            message: "Vous avez un bon surplus. Pensez à l'épargner pour les dépenses futures.",
            icon: '💰',
          });
        }
      } else if (balance < 0) {
        recommendations.push({
          type: 'warning',
          message: 'Attention ! Votre solde est négatif. Réduisez vos dépenses ou augmentez vos revenus.',
          icon: '⚠',
        });
      } else {
        recommendations.push({
          type: 'neutral',
          message: 'Votre budget est équilibré. Attention à ne pas dépasser vos revenus.',
          icon: '⚪',
        });
      }
